...
import { ArrowRightCircle, CheckCircle, XCircle, Send, RotateCcw } from 'lucide-react'
    ...
    },
    {
      ...
      cell: r => (
        <div className="flex items-center justify-end gap-1 flex-wrap">
          ...
          {/* Convertir a venta (solo aprobadas) */}
          {r.estado === 'aprobada' && (
            <button
              ...
              <ArrowRightCircle size={13} /> Convertir a venta
            </button>
          )}

          {/* Enlace a venta generada */}
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-100">
          <SearchBar value={search} onChange={v => { setSearch(v); setPage(1) }}
            placeholder="Buscar por N° cotización, cliente..." className="max-w-sm" />
        </div>
      ...
      <Modal open={estadoModal !== null} onClose={() => setEstadoModal(null)} title="Cambiar estado" size="sm">
        <p className="text-sm text-[#5F6B7A] mb-4">
          ¿Confirmar cambio de estado de <strong>{estadoModal?.cot.numero_cotizacion}</strong> a{' '}
      ...
      <Modal open={convertirCot !== null} onClose={() => setConvertirCot(null)} title="Convertir a venta" size="sm">
        <p className="text-sm text-[#5F6B7A] mb-4">
          Se creará una venta a partir de <strong>{convertirCot?.numero_cotizacion}</strong> y se descontará el stock.
